    ...
    public void runOpMode() {
        ...
        */
        ...
        Waypoint[] goToStackWaypoints = new Waypoint[] {
                ...
                new Waypoint(109, 36, PI/2, 40, 30, 0, goToStackTime),
        };
        ...
        double[][] wobbleDelivery = {{121, 93}, {97, 118}, {127, 130}};
        double[][] wobble2Delivery = {{116, 79}, {92, 109}, {118, 130}};
        double[] wobbleCor;
        double[] wobble2Cor;
        ...
        while (opModeIsActive()) {
            ...
            if (!goToStack) {
                ...
                if (time.seconds() > goToStackTime) {
                    if (ringCase != RingCase.Zero) {
                        ...
                        robot.thetaOffset = 0.06;
                        Constants.HIGH_GOAL_VELOCITY = 2050;
                        robot.highGoalShoot(ringCase == RingCase.Four ? 4 : 1, false);
                    } else {
            ...
            // Intake start stack
                ...
                if (ringCase == RingCase.Four) {
                    if (time.seconds() > intakeStackTime - 0.75) {
                        robot.intake.setPower(-0.75);
                        robot.intake.blockerDown();
                    } else if (robot.isAtPose(109, 38.0, PI/2, 0.5, 0.5, PI/35)) {
                        robot.intake.on();
                        robot.intake.setBlocker(0.51 - 0.12 * time.seconds());
                        robot.drivetrain.stop();
                    } else {
                        robot.setTargetPoint(109, 38.0, PI/2);
                    }
                }
                    ...
                    if (!psFinish) {
                        ...
                        if (!sweep) {
                            ...
                            Waypoint[] ringWaypoints = new Waypoint[] {
                                    new Waypoint(robot.x, robot.y, robot.theta, 60, 60, 0, 0),
                                    new Waypoint(64, 128, 0, 40, 30, 0, 2.0),
                                    new Waypoint(116, 129, 0, 40, 30, 0, 3.75),
                                    new Waypoint(127, 130, 0, 40, 5, 0, ringTime),
                            };
            ...
            // Deliver first wobble goal
                ...
                if (ringCase == RingCase.Four && sweep) {
                    ...
                    robot.setTargetPoint(new Target(curPose).thetaW0(curPose.theta + PI));
                }
                ...
                if ((reachedDeposit && time.seconds() > depositReachTime + 0.5) || time.seconds() > (ringCase == RingCase.Four ? 2.0 : 3.25)) {
                    ...
                    Waypoint[] intakeWobble2Waypoints = new Waypoint[] {
                            ...
                            new Waypoint(78, 23.5, PI/2, -0.1, 50, 0, 2.5),
                            new Waypoint(87, 23.5, PI/2, -0.1, -0.1, 0, intakeWobble2Time),
                    };
            ...
            // Go to pick up second wobble goal
                ...
                if (curTime < 2.5) {
                    robot.setTargetPoint(new Target(curPose).thetaW0(curPose.theta + PI).yKp(0.7));
                } else {
                    robot.setTargetPoint(new Target(curPose).thetaW0(PI/2).thetaKp(5.0));
                }
                ...
                if ((reachedDeposit && time.seconds() > depositReachTime + 0.5) || time.seconds() > 2.75) {
                    ...
                    if (ringCase == RingCase.Zero) {
                        parkWaypoints = new Waypoint[] {
                                ...
                                new Waypoint(79, 90, PI/3, 20, 20, 0, parkTime),
                        };
                    } else if (ringCase == RingCase.One) {
                        parkWaypoints = new Waypoint[] {
                                new Waypoint(robot.x, robot.y, robot.theta, -30, -30, 0, 0),
